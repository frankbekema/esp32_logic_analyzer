...
esp_err_t logic_analyzer_get_hw_param(logic_analyzer_hw_param_t *hw)
{
    ...
    if (hw->current_psram) // psram
    {
        if (hw->current_channels > 4)
        {
            ...
            hw->max_sample_cnt = (hw->current_channels == 8 ? la_frame.fb.len : la_frame.fb.len / 2);
...
static void logic_analyzer_clear(void)
{
    // free allocated mem
    if (la_frame.dma)
    {
...
#endif
            int l_samples = cfg->number_channels > 4 ? (cfg->number_of_samples * (cfg->number_channels / 8)) : (cfg->number_of_samples / 2);
...
static esp_err_t logic_allocate_dma_buffer(logic_analyzer_config_t *config)
{
    ...
    uint32_t bytes_to_alloc = config->number_channels > 4 ? (hw_param.max_sample_cnt * (config->number_channels / 8)) : (hw_param.max_sample_cnt / 2);
    if (config->samples_to_psram == 0)
    {
        // alloc on RAM
        uint32_t largest_free_block = heap_caps_get_largest_free_block(MALLOC_CAP_DMA); // byte
        uint32_t bytes_to_alloc = largest_free_block - ((largest_free_block / DMA_FRAME) + 2) * sizeof(lldesc_t); // free space with dma lldesc size
...
esp_err_t logic_analyzer_init(logic_analyzer_config_t *config)
{
    esp_err_t ret = ESP_OK;
    logic_analyzer_serial_init();

    ret = logic_allocate_dma_buffer(config);
    if(ret != ESP_OK)
    {
        return ret;
    }
